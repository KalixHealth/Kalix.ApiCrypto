...
public class JwtDecodeOptions<T>
{
    ...
    public JwtDecodeOptions(string token)
    {
        ...
        {
            throw new ArgumentException("Invalid JWT, does not have 2 or 3 parts", nameof(token));
        }
    ...
    private static string Verify(IDictionary<string, object> headers, ECDsa cert, string headerPart, string payloadPart, string signature)
    {
        if (cert == null)
        {
